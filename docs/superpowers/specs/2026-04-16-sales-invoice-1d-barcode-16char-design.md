...
- Human-readable display label: plain raw 16-char value (no grouping dashes).
- Barcode encodes the same raw 16-char value.
...
## Cutover
- 16-char format is mandatory from rollout.
- Dispatch and lookup logic accept only 16-char canonical codes.
- Generation logic creates only 16-char codes.
...
- Accept only 16-char canonical form.
...
- Dispatch accepts only 16-char code shape.
