...
| `scripts/run_ephemeral_e2e.sh`              | Strict bash + trap cleanup; ephemeral `asn-e2e-*` site; erpnext + asn_module; tolerant India fixtures; `allow_tests` + `before_tests`; `/etc/hosts`; serve + readiness HTTP check; `run-ui-tests` in `smoke` | `ci` modes.                                         |
