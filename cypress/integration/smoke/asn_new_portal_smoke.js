...
context("ASN New portal smoke", () => {
	...
	it("accepts valid single ASN submission", () => {
		...
		cy.get(".btn-primary").contains("Submit").click();
		cy.get(".alert-success, .portal-success, .scan-success", { timeout: 20000 }).should(
			"be.visible"
		);
	});
	...
	it("accepts valid bulk CSV", () => {
		...
		const csv = [
			...
			`BULK-${Date.now()},2026-04-01,2026-04-10,,,Transporter,KA01AB1234,,1500,${
				po.name
			},1,${po.items[0].item_code},1,100`,
		].join("\n");
